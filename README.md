...
| **Device** | ADB and fastboot detection, device properties, bootloader variables (slot, lock state, fastbootd, partition sizes), reboots into bootloader / fastbootd / recovery / download. Flashing is guarded: identity partitions refused outright, bootloader chain behind explicit opt-in, image-vs-partition size checks, dynamic partitions require fastbootd |
